...
git branch | grep -v alpha | grep -v beta | grep -v develop | grep -v main
if [ "$?" == "0" ]; then
  ...
  git branch | grep -v alpha | grep -v beta | grep -v develop | grep -v main | awk '{print "git branch -D " $1}' | sh
else
  exit 2
fi
